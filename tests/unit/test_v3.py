...
class TestSecrets(ZuulTestCase):
    ...
    def test_oidc_iss_overwrite(self):
        # Test that the 'iss' claim is overwritten or not correctly
        with open(os.path.join(
            FIXTURE_DIR,
            'config/secrets/git/',
            'org_project2/zuul-oidc-iss-overwrite.yaml')) as f:
            config = f.read()
        file_dict = {'zuul.yaml': config}
        A = self.fake_gerrit.addFakeChange('org/project2', 'master', 'A',
                                           files=file_dict)
        ...
        expected_isses = {
            ...
            "oidc_secret_iss_overwrite_allowed": "https://zuul.allowed.com",
            "oidc_secret_iss_overwrite_not_allowed": "https://zuul.example.com"
        }
        for secret_name, secret_content in secrets.items():
            self._validate_oidc_token(
                secret_name, secret_content.value, self.history[0].uuid,
