...
class TestLineMapping(AnsibleZuulTestCase):
    ...
    def test_line_mapping_large_change_no_rebase(self):
        ...
        content_lines_at_B = content_lines_at_A[:14]
        file_dict = {'README': "\n".join(content_lines_at_B)}
        B = self.fake_gerrit.addFakeChange('org/project', 'master', 'B',
                                           files=file_dict)
        
        # Delete line 15 to line 24 and then insert a line at line 15
        content_lines_at_C = content_lines_at_B + ["change_2"]
        file_dict = {'README': "\n".join(content_lines_at_C)}
        B = self.fake_gerrit.addFakeChange('org/project', 'master', 'C',
                                           files=file_dict)
