...
class TestGitlabDriver(ZuulTestCase):
    ...
    @simple_layout('layouts/branches-gitlab.yaml', driver='gitlab')
    def test_merge_request_matched_branch_event(self):
        """Test merge request targeting a branch that matches pipeline configuration"""
        A = self.fake_gitlab.openFakeMergeRequest('org/project', 'master', 'A')
        self.fake_gitlab.emitEvent(A.getMergeRequestOpenedEvent())
        self.waitUntilSettled()
    ...
    def test_merge_request_unmatched_branch_event(self):
        """Test merge request targeting a branch that doesn't match pipeline configuration"""
