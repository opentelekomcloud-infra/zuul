...
class TestScaleOutScheduler(ZuulTestCase):
    ...
    def test_change_cache_delay(self):
        ...
        def s2_review(*args, **kw):
            ...
            # We don't normally automatically emit the change-merged
            # event in tests in order to keep them simple.  But we
            # need to in this case so that we can trigger a change
            # query that races the isMerged query.
