    ...
    def getFilesChanges(self, branch, tosha=None, zuul_event_id=None):
        with self.createRepoObject(zuul_event_id) as repo:
            ...
            if tosha:
                ...
                diff_output = repo.git.diff(commit_diff, name_only=True)
                diff_files = set(f for f in diff_output.splitlines())

                log_output = repo.git.log(
                    commit_diff,
                    name_only=True, pretty="format:", no_merges=True)
                files.update(
                    f for f in log_output.splitlines() if f in diff_files)
            else:
