...
class PeekQueue:
    ...
    def _get_project_and_ref(self, data):
        ...
        if kind == 'ref-updated':
            ...
            # TODO the project info is top level in the event too but our
            # test events don't contain it.
    ...
    def run(self):
        ...
        while new_event_list:
            ...
            data = event["payload"]
            project, ref = self._get_project_and_ref(data)
            ok = False
            delay = None
            now = time.time()
...
class GerritEventProcessor:
    ...
    def _handleEvent(self, connection_event):
        ...
        # in preprocessing.
        for event_filter in self.connection.watched_event_filters:
