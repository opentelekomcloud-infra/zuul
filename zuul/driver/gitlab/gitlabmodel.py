...
class GitlabRejectFilter(RefFilter):
    ...
    def matches(self, change):
        # Break if any of the defined rejection conditions is not met...
        if any([
            self.open is not None and change.open != self.open,
            self.merged is not None and change.is_merged != self.merged,
            self.approved is not None and change.approved != self.approved,
            self.labels and set(self.labels).isdisjoint(set(change.labels)),
        ]):
