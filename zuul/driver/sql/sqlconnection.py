...
POSTGRES_STATEMENT_TIMEOUT_RE = re.compile(r'/\* postgres_statement_timeout=(\d+) \*/')
MARIADB_STATEMENT_TIMEOUT_RE = re.compile(r'/\* mariadb_statement_timeout=(\d+) \*/')
...
def _set_timeout(conn, cursor, stmt, params, context, executemany,
    ...
    if match:
        ...
        if match:
            stmt = (f"SET STATEMENT max_statement_time={match.groups()[0]/1000} "
                    f"for {stmt}")
    return (stmt, params)
...
class DatabaseSession(object):
    ...
    def getBuildsets(self, tenant=None, project=None, pipeline=None,
        ...
        if not (project or change or uuid):
            ...
            q = q.with_hint(buildset_ref_table, 'USE INDEX (PRIMARY)', 'mariadb')
