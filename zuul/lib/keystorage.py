...
class KeyStorage(ZooKeeperBase):
    ...
    def rotateOidcSigningKeys(self, algorithm, rotation_interval, max_ttl):
        ...
        if not key_data:
            ...
            if age_seconds > rotation_interval:
                ...
                key_data.appendKey(latest_key["version"] + 1, int(time.time()), private_key)
                update_required = True
    ...
    def _createAndStoreOidcSigningKeys(self, algorithm):
        ...
        private_key = self._generateOidcSigningKey(algorithm)
        OIDCKeys.new(self.zk_context, algorithm, 0, int(time.time()), private_key)
    ...
    def _loadOidcSigningKeys(self, algorithm):
        ...
        key_path = self._getOidcSigningKeysPath(algorithm)
        try:
            return OIDCKeys.fromZK(self.zk_context, key_path, algorithm=algorithm)
        except kazoo.exceptions.NoNodeError:
            return None
