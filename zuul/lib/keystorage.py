...
class OIDCSigningKeysCache(ZuulTreeCache):
    ...
    def __init__(self, client):
        super().__init__(
            client, OIDCKeys.OIDC_ROOT_PATH, async_worker=False)

    def objectFromRaw(self, key, data, zstat):
        return OIDCKeys._fromRaw(data, zstat, None)
...
class OIDCKeys(ZKObject):
    ...
    def createAndStoreOidcSigningKeys(
        ...
        private_key = klass._generateOidcSigningKey(algorithm, password_bytes)
        klass.new(context, algorithm, private_key)

    @classmethod
    def _generateOidcSigningKey(klass, algorithm, password_bytes):
        if algorithm == "RS256":
        ...
        if not key_data:
            ...
            with key_data.activeContext(context):
                ...
                if age_seconds > rotation_interval:
                    ...
                    private_key = OIDCKeys._generateOidcSigningKey(
