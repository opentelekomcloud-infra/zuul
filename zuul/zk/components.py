    ...
    def __setattr__(self, name, value):
        ...
        with self.register_lock:
            ...
            try:
                ...
                self.log.error("Could not update %s in ZooKeeper: "
                               "node does not exist", self)
            # we log and re-raise every other type of exception to give
            # a chance to the original setter to handle the issues
                ...
                self.log.error(
                    "Unknown error encountered while updating "
                    "%s in Zookeeper: %s", self, e
                )
                raise
