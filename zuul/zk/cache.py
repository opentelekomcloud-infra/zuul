...
class ZuulTreeCache(abc.ABC):
    ...
    def _handleCacheEvent(self, event):
        ...
        key, shouldFetch = self.parsePath(event.path)

        if ((not shouldFetch) and event.type != EventType.NONE
    ...
    def parsePath(self, path):
        ...
        Returns a tuple: (key, shouldFetch)

        Return a key of None to indicate the path is not relevant to the cache.
        The shouldFetch return value indicates whether the cache should
