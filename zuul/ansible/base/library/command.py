...
class StreamFollower:
    def __init__(self, cmd, log_uuid, output_max_bytes, no_log=False):
        self.cmd = cmd
    ...
    def follow_inner(self, fd, log_bytes):
        self.console.addLine('[Zuul] follow_inner %s\n' % repr(fd))
        if self.no_log:
            self.console.addLine('[Zuul] follow_inner - no-log %s\n' % repr(fd))
            data = fd.read()
            self.console.addLine('[Zuul] follow_inner read %d\n' % len(data))
            return
...
def zuul_run_command(self, args, zuul_log_id, zuul_ansible_split_streams, zuul_output_max_bytes, check_rc=False, close_fds=True, executable=None, data=None, binary_data=False, path_prefix=None, cwd=None,
    ...
    try:
        ...
        if self.no_log:
            # Drain buffers manually to avoid deadlock
            follower = StreamFollower(cmd, zuul_log_id, zuul_output_max_bytes, no_log=True)
            follower.follow()
        else:
