...
class ProcessSensor(SensorInterface):
    ...
    def isOk(self):
        ...
        # but containers do.
        # If no max is found assume infinity. If no current usage is found
        # assume 1 for the current process.
        root_max = self._get_root_cgroup_max() or math.inf
        root_current = self._get_root_cgroup_current() or 1
        # Processes running under systemd will have these values.
        user_max = self._get_user_slice_max() or math.inf
        ...
        if self.statsd:
            # TODO(clarkb) can graphite/statsd handle math.inf?
            self.statsd.gauge(self.base_key + '.max_process',
                              limit)
            self.statsd.gauge(self.base_key + '.cur_process',
                              usage)

        # When headroom is infinite we are never using too many processes.
        # Also the math gets weird so just short circuit.
        if headroom != math.inf and min_headroom >= headroom:
