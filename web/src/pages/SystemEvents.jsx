...
import { fetchSystemEvents } from '../api'
import {
  makeQueryString,
  getFiltersFromUrl,
  writeFiltersToUrl,
} from '../containers/FilterToolbar'
import SystemEventTable from '../containers/systemevents/SystemEventTable'
...
class SystemEventsPage extends React.Component {
  ...
  componentDidMount () {
    document.title = 'Zuul System Events'
  ...
  updateData = (filters) => {
    ...
    // the table instead?
    fetchSystemEvents(this.props.tenant.apiPrefix, queryString).then((response) => {
  ...
  render () {
    ...
    const { events, fetching, filters, resultsPerPage, currentPage, itemCount } = this.state
