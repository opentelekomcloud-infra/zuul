...
That reference is by name, which means that a given label is always
the same combination of image and flavor objects in every provider
that uses it.  It is, however, possible for the underlying
characteristics of those images and flavors to be different in
different tenants since different tenants may have different image and
flavor objects with the same name.
