...
1. A collection of private data for use by one or more jobs,
   hereafter referred to as a :def:`data secret`.

2. Automatic OIDC token generation for use by jobs, hereafeter
   referred to as a :def:`token secret`.
...
A :def:`data secret` is one where values are specified using the
...
A :def:`token secret` is one where an OpenID token is automatically
generated for the job being run.  Configuration of the token is
described below in :attr:`secret.oidc`.
...
.. attr:: secret
   ...
   .. attr:: data
      ...
      Use of this attribute generates a :def:`data secret`.
   ...
   .. attr:: oidc
      ...
      Use of this attribute generates a :def:`token secret`.
      ...
      .. attr:: ttl
         ...
         value is :attr:`tenant.default-oidc-ttl`.
