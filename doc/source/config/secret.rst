...
Zuul support secret encryption and generation of OIDC ID tokens.
...
Signing key rotation is handled by Zuul automatically and the rotation
interval can be configured in :attr:`oidc.signing_key_rotation_interval`
in Zuul configuration. 

In case a key is compromised, zuul-admin command ``delete-oidc-signing-keys``
can be used to delete the signing keys of a specific algorithm then Zull
...
.. attr:: secret
   ...
   .. attr:: oidc
      ...
      variables available to the job.  It can be used to exchange an
      access token for external services that trust Zuul.
