...
A provider is responsible for managed test worker nodes for Zuul.  A
...
image.  The resource limits will still apply, even if a flavor in one
tenant uses 2 cores and the same flavor name in another tenant uses 4.
...
represents a portion of a cloud or other system that supplies test
...
which represent the entire cloud or test resource supplier.
